...
### Scope
- Baseline release: `1.1.5` (2026-04-30).
- Major refactor: pluggable `modules/` subsystem replacing the in-tree dashboard add-ons
  (`dashboard/rexgend_router.py` + `dashboard/templates_rexgen/` and
  `dashboard/xoraya_router.py` + `dashboard/templates_xoraya/`).
- New first-party module: **xoraya** (browser-based Xoraya logger control plane).
- New release tooling: `scripts/pack.sh` for reproducible deploy/source tarballs.
- Deploy pipeline switched from flat `scp -r` to `rsync` with explicit excludes.
- Runtime version bump:
  - `DASHBOARD_VERSION: 1.1.5 -> 1.2.0`.

### Added
- **`modules/` pluggable subsystem** — new container for self-contained add-on
  Flask blueprints. Two modules ship in 1.2.0:
  - `modules/rexgen/` — extracted from the legacy `dashboard/rexgend_router.py`
    plus `dashboard/templates_rexgen/`. Self-contained router, templates, and
    runtime state.
  - `modules/xoraya/` — new module providing a browser dashboard for the
    Xoraya datalogger (15 interface tabs, 42 SDK bridge modes, full
    properties read/write, HDD download, live capture, system/storage/network/
    admin APIs). Versioned independently (`xoraya 1.0.0`); see
    `modules/xoraya/docs/CHANGELOG.md` for the per-module changelog.
- **`scripts/pack.sh`** — release tooling. Produces:
  - `dist/netservices-<ver>-deploy.tar.gz` (~700 KB) — runtime only, drops
    SDK staging trees, .cpp source, build scripts, dev docs, caches. Used
    for copying onto a ReXgen device.
  - `dist/netservices-<ver>-source.tar.gz` — full tree for archival.
  - Each tarball is accompanied by a `.sha256` sidecar. Filenames track the
    `DASHBOARD_VERSION` constant automatically.
- **rsync-based deploy in `scpme.sh`** — replaces flat `scp -r` with
  `rsync -a` plus explicit excludes (`modules/xoraya/sdk/`, `*.cpp`,
  `build_sdk_if_bridge.sh`, `__pycache__/`, `*.pyc`, `.svn/`). Cuts on-device
  module footprint from ~57 MB to ~1.3 MB.
- **`svn:ignore` for `dist/`** — generated archives no longer pollute
  `svn status`.

### Changed
- **Dashboard `app.py` integration model** for add-on routers:
  - Removed direct import of `rexgend_router` and `xoraya_router` modules
    sitting next to `app.py`.
  - Modules are now discovered through `netservices.modules.<name>.router`
    package path. Each module exposes `router = Blueprint(...)` and is
    `register_blueprint`-ed during app construction.
- **Templates ownership**:
  - rexgen templates moved out of `dashboard/templates_rexgen/` into
    `modules/rexgen/...` (owned by the module).
  - xoraya templates owned entirely under `modules/xoraya/src/xoraya/dashboard/templates/`.
  - `dashboard/templates_main/` continues to own the netservices core pages
    (Wi-Fi/AP/System/Device).
- **`_topnav.html`** simplified — module-aware entries are now generated by
  registered modules rather than hardcoded in the core navigation partial.
- **`_footer.html`** — version readout aligned with the new
  `DASHBOARD_VERSION = "1.2.0"` constant.

### Removed
- `dashboard/rexgend_router.py` — replaced by `modules/rexgen/router.py`.
- `dashboard/rexgen_constants.py` — moved into `modules/rexgen/`.
- `dashboard/templates_rexgen/` — moved into `modules/rexgen/`.
- `dashboard/xoraya_router.py` — replaced by `modules/xoraya/router.py`.
- `dashboard/templates_xoraya/` — replaced by templates inside
  `modules/xoraya/src/xoraya/dashboard/templates/`.

### Architecture
- Single Flask runtime in `dashboard/app.py` retained.
- Module loading order is deterministic; failures in one module do not affect
  the others (each `register_blueprint` is wrapped).
- Modules carry their own `pyproject.toml`, `docs/`, and (for native bridges)
  build infrastructure; netservices core does not need to know module internals.

### Deployment changes
- `scpme.sh` skeleton:
  ```bash
  rsync -a --rsh="sshpass ... ssh ..." \
      --exclude='modules/xoraya/sdk/' \
      --exclude='**/__pycache__/' \
      --exclude='**/*.pyc' \
      --exclude='**/.svn/' \
      services dashboard modules systemd scripts ssl install.sh \
      root@<ip>:$DEST/
  ```
- One-shot module install path is `scripts/pack.sh deploy` + scp tarball +
  `bash xoraya/scripts/install.sh` on the device.

### File-level delta vs 1.1.5
- **Modified**
  - `dashboard/app.py` (version bump + module registration via `modules/` package)
  - `dashboard/templates_main/_footer.html` (version readout)
  - `dashboard/templates_main/_topnav.html` (module-aware navigation)
  - `scpme.sh` (rsync-based deploy with excludes)
- **Added**
  - `modules/__init__.py`
  - `modules/rexgen/...`
  - `modules/xoraya/...` (see `modules/xoraya/docs/CHANGELOG.md` for full
    sub-tree)
  - `scripts/pack.sh`
- **Removed**
  - `dashboard/rexgend_router.py`
  - `dashboard/rexgen_constants.py`
  - `dashboard/templates_rexgen/`
  - `dashboard/xoraya_router.py`
  - `dashboard/templates_xoraya/`

### Compatibility
- No public API URL changes for netservices core. Wi-Fi/AP/System/Device routes
  unchanged.
- Module URL prefixes documented in each module's `README.md` /
  `RELEASE_NOTES.md` (e.g. xoraya is mounted under `/xoraya`).
- `netservices.conf` schema unchanged.

## 1.1.5 (2026-04-30)

### Scope
- Baseline release: `1.1.4` (2026-03-23).
- This release documents stability/maintenance deltas and aligns release metadata for ongoing dashboard/runtime split work.

### Changed
- Release documentation refreshed for current workspace state.
- Operational notes clarified for split router/template architecture (`templates_main` + `templates_rexgen`).

### Stability
- No intentional breaking API contract changes in existing dashboard endpoints.
- Focus remains on runtime consistency and maintainability between `dashboard` and service-layer modules.
